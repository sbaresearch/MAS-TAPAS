...
class DOMIAS(BaseAttacker):
    ...
    def _compute_attack_scores(
        ...
        de_synth = self._fit_estimator(synth,'synth')
        de_ref = self._fit_estimator(ref,'ref')

        P_s = self._compute_density(X_test, de_synth,'synth')
        P_r = self._compute_density(X_test, de_ref,'ref')
